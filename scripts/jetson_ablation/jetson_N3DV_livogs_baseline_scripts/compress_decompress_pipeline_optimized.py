...
def save_queen_ply(params, output_path, sh_degree=2, eps=1e-6):
    ...
    If the decoded colors have fewer SH channels than sh_degree requires,
    missing higher-order coefficients are zero-padded so the PLY always
    matches the expected QUEEN attribute layout.
    """
    ...
    n_dc = 3
    n_rest_target = 3 * ((sh_degree + 1) ** 2 - 1)
    n_rest_actual = colors.shape[1] - n_dc

    sh_dc = colors[:, :n_dc]
    if n_rest_actual >= n_rest_target:
        sh_rest = colors[:, n_dc:n_dc + n_rest_target]
    else:
        sh_rest_data = colors[:, n_dc:] if n_rest_actual > 0 else np.zeros((N, 0), dtype=np.float32)
        sh_rest = np.concatenate([
            sh_rest_data,
            np.zeros((N, n_rest_target - n_rest_actual), dtype=np.float32),
        ], axis=1)
    n_rest = n_rest_target
...
if __name__ == "__main__":
    ...
    for frame in tqdm(range(args.frame_start, args.frame_end + 1, args.interval), desc="Frames"):
        ...
        if args.output_ply_folder is not None:
            ...
            save_queen_ply(decoded_params, ply_out_path)
