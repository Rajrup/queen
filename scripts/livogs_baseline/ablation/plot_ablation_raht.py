...
PHASES         = ["prelude_ms", "forward_ms", "inverse_ms"]
PHASE_LABELS   = {"prelude_ms": "Prelude", "forward_ms": "Forward", "inverse_ms": "Inverse"}

# (sh_key, variant) → phase → colour
# SH0: blue family  |  SH*: orange family
PALETTE: dict[tuple[str, str], dict[str, str]] = {
    ("sh0",    "pytorch"): {"prelude_ms": "#c5d9f0", "forward_ms": "#4c72b0", "inverse_ms": "#2a4a7f"},
    ("sh0",    "cuda"):    {"prelude_ms": "#ffd5b5", "forward_ms": "#dd8452", "inverse_ms": "#a05a2c"},
    ("shstar", "pytorch"): {"prelude_ms": "#b8ddb8", "forward_ms": "#55a868", "inverse_ms": "#2d6e3a"},
    ("shstar", "cuda"):    {"prelude_ms": "#e5b5c5", "forward_ms": "#c44e52", "inverse_ms": "#8b2e32"},
}

# single-SH palette (no SH dimension)
PALETTE_SINGLE = {
    "pytorch": {"prelude_ms": "#aec6e8", "forward_ms": "#4c72b0", "inverse_ms": "#2a4a7f"},
    "cuda":    {"prelude_ms": "#ffc09f", "forward_ms": "#dd8452", "inverse_ms": "#a05a2c"},
}
...
DECODE_PHASES = ["prelude_ms", "inverse_ms"]
...
def plot_latency_combined(
    ...
    """4 stacked bars per sequence (decode only: Prelude + Inverse)."""
    ...
    for b_idx, (sh_key, variant) in enumerate(BAR_KEYS):
        ...
        for s_idx in range(n_seq):
            ...
            # Stack: prelude at bottom, inverse on top
            bottom = 0.0
            ...
            total = prelude_val + inverse_val

            # Total on top
            ax.text(x_pos[s_idx], total + 0.3, f"{total:.0f}",
                    ha="center", va="bottom", fontsize=6.5, fontweight="medium")

            # If prelude is shorter than inverse, label the inverse portion
            if prelude_val < inverse_val:
                inv_mid_y = prelude_val + inverse_val / 2.0
                ax.text(x_pos[s_idx], inv_mid_y, f"{inverse_val:.0f}",
...
def print_summary_combined(
    ...
    for s_idx, label in enumerate(seq_labels):
        for sh_key, sh_label, all_means in [("sh0", "SH0", sh0_means),
            ...
            m_cu = all_means[s_idx].get("cuda",    {})
            if m_py and m_cu:
                spd = lambda py, cu: f"{py / cu:>9.2f}x" if cu > 0 else f"{'N/A':>10s}"
                tot_py = sum(m_py.get(ph, 0.0) for ph in PHASES)
                tot_cu = sum(m_cu.get(ph, 0.0) for ph in PHASES)
                print(f"{'':18s}  {sh_label:<5s}  {'→ speedup':<10s}  "
                      f"{spd(m_py.get('prelude_ms', 0), m_cu.get('prelude_ms', 1))}  "
                      f"{spd(m_py.get('forward_ms', 0), m_cu.get('forward_ms', 1))}  "
                      f"{spd(m_py.get('inverse_ms', 0), m_cu.get('inverse_ms', 1))}  "
                      f"{spd(tot_py, tot_cu)}")
        print()
...
def plot_latency_single(
    ...
    ax.set_title("RAHT Latency: PyTorch vs CUDA — Prelude / Forward / Inverse",
                 fontsize=12, fontweight="bold")
