...
def main() -> None:
    ...
    frame0_str = str(args.frame_start).zfill(4)
    ...
    params_wu, _ = load_queen_ply(ply_file, device=device)
    ...
    for frame in tqdm(frames, desc="Frames"):
        ...
        if ply_file is None:
            tqdm.write(f"  WARNING: PLY not found for frame {frame_str}, skipping")
            continue
