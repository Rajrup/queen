"""
Plot MesonGS compressed size and point counts per frame.
Requires: benchmark_mesongs.csv under input_folder/<config_name>/

Generates two plots:
  1. Size: uncompressed vs total compressed per frame
  2. Point counts: original, after-prune, after-octree, decoded
"""
import os
import argparse
import csv
    ...
    with open(path) as f:
        ...
        for row in r:
            rows.append({
                ...
                "after_prune_points": int(row["after_prune_points"]),
                "after_octree_points": int(row["after_octree_points"]),
            })
...
def main():
    ...
    ax.plot(x, total_mb, "s-", label="MesonGS compressed", color="coral", markersize=3)

    ax.set_xlabel("Frame")
    ax.set_ylabel("Size (MB)")
    ax.set_title(f"MesonGS size per frame [{args.config_name}, {args.dataset_name}, {args.sequence_name}]")
    ...
    prune_pts = [r["after_prune_points"] for r in rows]
    octree_pts = [r["after_octree_points"] for r in rows]

    pruning_active = any(o != p for o, p in zip(orig_pts, prune_pts))

    fig, ax = plt.subplots(figsize=(12, 5))
    ax.plot(x, orig_pts, "o-", label="Original", color="steelblue", markersize=3)
    if pruning_active:
        ax.plot(x, prune_pts, "^-", label="After prune", color="seagreen", markersize=3)
    ax.plot(x, octree_pts, "s-", label="After octree", color="coral", markersize=3)
    ...
    avg_oct = sum(octree_pts) / n
    oct_ratio = avg_oct / avg_orig * 100 if avg_orig > 0 else 0
    ann = f"avg original={avg_orig:.0f}, after octree={avg_oct:.0f} ({oct_ratio:.1f}%)"
    if pruning_active:
        avg_prune = sum(prune_pts) / n
        prune_ratio = avg_prune / avg_orig * 100 if avg_orig > 0 else 0
        ann = f"avg original={avg_orig:.0f}, pruned={avg_prune:.0f} ({prune_ratio:.1f}%), octree={avg_oct:.0f} ({oct_ratio:.1f}%)"
    ax.annotate(
        ann,
        xy=(0.02, 0.95), xycoords="axes fraction",
        fontsize=9, va="top",
