#!/usr/bin/env python3
"""Latency and resource benchmark for 4 compression pipelines across Neural_3D_Video sequences.

Runs LiVoGS, VideoGS, DracoGS, MesonGS sequentially on QUEEN-trained models
while monitoring CPU, RAM, and GPU resources at 30ms intervals. No quality evaluation.
"""
...
def _mesongs_args(ply_path: str, output_folder: str, seq: str,
                  frame_start: int, frame_end: int, interval: int) -> list[str]:
    dataset_path = str(
        Path(DATA_PATH_GLOBAL) / DATASET_NAME / seq
    )
    return [
        "--ply_path", ply_path,
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--frame_start", type=int, default=None,
                   help="Override frame start for all pipelines (default: 1)")
    p.add_argument("--frame_end", type=int, default=None,
                   help="Override frame end for all pipelines (default: 300)")
...
def main() -> None:
    ...
    frame_start_override = args.frame_start if args.frame_start is not None else 1
    frame_end_override = args.frame_end if args.frame_end is not None else 300

    monitor = None if args.dry_run else ResourceMonitor(gpu_index=gpu_id)

    run_start = time.time()
    log_header("Latency & Resource Benchmark (QUEEN / Neural_3D_Video)")
    ...
    for seq in args.sequences:
        ...
        gt_model_path = str(
            Path(args.data_path) / "pretrained_output" / DATASET_NAME
        ...
        for pipeline_name in args.pipelines:
            ...
            pipeline_args = cfg.build_args_fn(
                gt_model_path, output_dir, seq, frame_start, frame_end, interval
            )
            cmd = [
            ...
            num_frames = len(range(frame_start, frame_end + 1, interval))
            print(f"\n--- {pipeline_name.upper()} | {seq} | "
                  f"frames {frame_start}-{frame_end} (interval={interval}, "
