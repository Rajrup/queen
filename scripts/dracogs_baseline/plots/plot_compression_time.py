"""
Stacked area plot of DracoGS encode and decode time per frame.
...
def main():
    ...
    ax.stackplot(
        ...
        encode_ms,
        decode_ms,
        labels=["Encode (DracoGS)", "Decode (DracoGS)"],
        colors=["steelblue", "coral"],
        alpha=0.8,
    )
    ax.set_xlabel("Frame")
    ax.set_ylabel("Time (ms)")
    ax.set_title(f"DracoGS encode + decode time per frame [{args.config_name}, {args.dataset_name}, {args.sequence_name}]")
